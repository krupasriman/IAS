...
export default function HomePage() {
	...
	const [webSearchEnabled, setWebSearchEnabled] = useState(() => {
		try {
			return localStorage.getItem("ias_web_search_enabled") === "true";
		} catch {
			return false;
		}
	});
	...
	const handleToggleWebSearch = (enabled: boolean) => {
		...
		try {
			localStorage.setItem("ias_web_search_enabled", String(enabled));
		} catch {}
	};
								...
								<div className="grid sm:grid-cols-2 gap-3">
									{webSearch.searchResults.results.slice(0, 8).map((r, i) => (
										...
										>
											...
											<p className="text-sm font-semibold text-slate-800 line-clamp-2 group-hover:text-blue-700">
												{r.title}
											</p>
				...
				<div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-300">
					...
					<p className="text-sm text-slate-400 mt-1 mb-4">
						{query
							...
							: "Try a different category or add your own topic"}
					</p>
