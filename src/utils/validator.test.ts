...
const makeTopic = (overrides: Partial<Topic> = {}): Topic => ({
  ...
  conclusion: {
    negative: 'Judicial overreach and enormous case pendency remain persistent structural challenges.',
    positive: 'However, judicial review remains the bedrock of the Indian constitutional democracy.',
  },
  source: 'local',
...
describe('validateTopic', () => {
  ...
  it('flags wrong number of pros and cons', () => {
    ...
    const report = validateTopic(makeTopic({ pros: base.pros.slice(0, 2), cons: base.cons.slice(0, 2) }));
  ...
  it('flags over-long explanations and examples', () => {
    ...
    expect(report.warnings.some((w) => w.includes('explanation has 30 words'))).toBe(true);
  ...
  it('flags a single-line string conclusion', () => {
    const report = validateTopic(makeTopic({ conclusion: 'just one line here' }));

    expect(report.warnings.some((w) => w.includes('1 lines (expected 2)'))).toBe(true);
