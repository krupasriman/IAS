...
export default function TopicCard({ topic }: TopicCardProps) {
  ...
  return (
    ...
    >
      {/* Top accent */}
      <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${categoryInfo.gradient}`} />

      <div className="flex items-start justify-between gap-3 mb-3">
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${categoryInfo.bgLight}`}>
      ...
      <p className="text-sm text-slate-500 mb-4 line-clamp-2 flex-1">{preview}...</p>
