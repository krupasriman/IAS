...
export default function ModelCombobox({
	...
	const currentVariantOption = currentOption?.variants?.find(
		(v) => v.id === currentVariant,
	);
	const hasVariants =
		currentOption?.variants && currentOption.variants.length > 0;

	const displayValue = currentOption
		? (currentOption.name !== currentOption.id
				? `${currentOption.name} (${currentOption.id})`
				: currentOption.id) +
	...
	return (
		<div className={`relative ${className}`}>
			<div ref={dropdownRef} className="relative">
				{/* biome-ignore lint/a11y/noStaticElementInteractions: wrapper expands click target; input inside handles focus/keyboard */}
				<div
