...
export async function fetchOpenRouterModels(apiKey?: string, forceRefresh = false): Promise<OpenRouterModelInfo[]> {
  if (!forceRefresh) {
    try {
  ...
  const models: OpenRouterModelInfo[] = rawList
    ...
    .map((m: any) => {
      ...
      const isFree = (promptCost === 0 && completionCost === 0 && requestCost === 0 && imageCost === 0);
  ...
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify({ timestamp: Date.now(), data: models }));
  } catch {
...
export async function fetchGeneralComputeModels(apiKey?: string, forceRefresh = false): Promise<GeneralComputeModelInfo[]> {
  if (!forceRefresh) {
    try {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to fetch General Compute models (${res.status} ${res.statusText})`);
  }
  ...
  const models: GeneralComputeModelInfo[] = rawList
    ...
    .map((m: any) => {
      ...
      return {
        ...
        name: m.name || m.id,
        description: m.description || '',
        contextLength: m.context_length ?? m.max_tokens ?? 0,
        isFree,
        pricing: {
          prompt: promptCost,
    ...
    .sort((a: GeneralComputeModelInfo, b: GeneralComputeModelInfo) =>
      Number(b.isFree ?? 0) - Number(a.isFree ?? 0) || a.name.localeCompare(b.name)
    );

  try {
    localStorage.setItem(GENERAL_COMPUTE_CACHE_KEY, JSON.stringify({ timestamp: Date.now(), data: models }));
  } catch {
    // Ignore cache write errors
  }
...
export async function fetchGoogleModels(apiKey?: string, forceRefresh = false): Promise<GoogleModelInfo[]> {
  if (!forceRefresh) {
    try {
  ...
  const models: GoogleModelInfo[] = rawList
    ...
    .sort((a: GoogleModelInfo, b: GoogleModelInfo) => a.displayName.localeCompare(b.displayName));

  try {
    localStorage.setItem(GOOGLE_CACHE_KEY, JSON.stringify({ timestamp: Date.now(), data: models }));
  } catch {
    // Ignore cache write errors
  }
  ...
  return [
    {
      ...
      description: 'Fast, versatile model with 1M token context. Free tier: 1500 req/min.',
    ...
    },
    {
      ...
      description: 'Advanced reasoning with 2M token context. Free tier: 50 req/min.',
    ...
    },
    {
      ...
      description: 'Next-gen model with improved speed and capabilities. Free tier available.',
      inputTokenLimit: 1048576,
      outputTokenLimit: 8192,
      supportedGenerationMethods: ['generateContent', 'streamGenerateContent']
    },
    {
      ...
      description: 'Lightweight model optimized for cost-efficiency. Free tier available.',
      inputTokenLimit: 1048576,
      outputTokenLimit: 8192,
      supportedGenerationMethods: ['generateContent', 'streamGenerateContent']
    },
