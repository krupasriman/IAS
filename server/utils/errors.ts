...
export function sendError(res: Response, status: number, error: string, details?: Record<string, string[]>): void {
  const body: ErrorResponse = { error };
  if (details) body.details = details;
  res.status(status).json(body);
}

export function sendValidationError(res: Response, error: string, details: Record<string, string[]>): void {
